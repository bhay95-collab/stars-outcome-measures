...
import { calcChange, getMCIDStatus, resolveMCIDEntry, MEASURES } from './clinical'
 ...
 */
export function buildFollowUpComparison(sourceAssessment, completedAssessment, condition = null) {
  ...
  const mcid = measure.mcidKey ? getMCIDStatus(measure.mcidKey, current, previous, condition) : null
  const threshold = measure.mcidKey ? resolveMCIDEntry(measure.mcidKey, condition)?.thresh ?? null : null
...
export function sanitizeFollowUpAssessment(assessment, sourceAssessment = null, condition = null) {
  if (!assessment) return null
  const measureId = assessment.measure
  const comparison = buildFollowUpComparison(sourceAssessment, assessment, condition)
  return {
    id: assessment.id,
...
export function shapeFollowUpRecord(request, response = null, assessment = null, now = new Date(), sourceAssessment = null, condition = null) {
  ...
  return {
    ...
    assessment: sanitizeFollowUpAssessment(assessment, sourceAssessment, condition),
  }
}

export function getFollowUpRecordAttention(record) {
  if (record?.reviewed_at) return FOLLOWUP_ATTENTION.GREEN
