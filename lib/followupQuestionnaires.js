...
export const FOLLOWUP_QUESTIONNAIRE_MEASURE_IDS = ['ABC', 'FSS', 'HADS', 'PDQ8', 'RPQ', 'BIVI', 'NPRS', 'LEFS', 'BPFS', 'KOOS', 'HOOS', 'HAGOS', 'CAIT', 'ATRS', 'FABQ', 'OMAS']
...
const QUESTIONNAIRES = {
  ...
  HAGOS: {
    id: 'HAGOS',
    instructions: 'Answer every question thinking of your hip and/or groin during the last week.',
    questions: koosFamilyQuestions(HAGOS_SECTIONS),
    calc: koosFamilyCalc(HAGOS_SECTIONS, calcHAGOS),
  },
  OMAS: {
    id: 'OMAS',
    instructions: 'For each item, choose the response that best describes your ankle now.',
    questions: numberedQuestions(OMAS_ITEMS, item => ({
      label: item.label,
      type: 'select',
      options: item.options.map(opt => ({ value: opt.value, label: `${opt.label} (${opt.value})` })),
    })),
