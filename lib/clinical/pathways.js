...
/**
 * Caseload-wide due signals for the directory panel.
 * Uses next_assessment_at when set; falls back to created_at + reassessmentDays.
 */
export function buildCaseloadDueSignals(patients = [], assessments = [], options = {}) {
  const now = options.now ? new Date(options.now) : new Date()
  ...
  for (const patient of patients) {
    const patientAssessments = byPatient.get(patient.id) ?? []
    const pathway = buildPatientPathway(patient, patientAssessments, options)
    if (!pathway.recommendedMeasures?.length) continue

    const latestByMeasure = latestAssessmentByMeasure(patientAssessments)
    const patientLabel = patient.label ?? patient.initials ?? 'Patient'

    for (const item of pathway.recommendedMeasures) {
      if (!item.hasBaseline) continue

      const latest = latestByMeasure.get(item.id)
      const explicitDue = latest?.next_assessment_at ? new Date(latest.next_assessment_at) : null

      if (explicitDue) {
        if (explicitDue <= now) {
          signals.push({ patientId: patient.id, patientLabel, measureId: item.id, measureName: item.name, daysSinceLast: item.daysSinceLast, lastRecordedLabel: item.lastRecordedLabel })
        }
      } else if (item.isDue) {
        signals.push({ patientId: patient.id, patientLabel, measureId: item.id, measureName: item.name, daysSinceLast: item.daysSinceLast, lastRecordedLabel: item.lastRecordedLabel })
      }
  ...
  return signals.sort((a, b) => (b.daysSinceLast ?? 0) - (a.daysSinceLast ?? 0))
}
