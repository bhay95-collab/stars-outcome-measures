...
export function buildPatientPathway(patient, assessments = [], options = {}) {
  ...
  } else if (!recommendedMeasures.length) {
    nextActions = [{
      type: 'pathway',
      tone: 'neutral',
      measureId: null,
  ...
  const statusTone = !diagnosis.raw || !recommendedMeasures.length
    ? 'neutral'
    : dueMeasures.length
      ? 'due'
