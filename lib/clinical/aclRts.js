...
//   - Landing mechanics LESS < 5 (Padua 2009 development; prospective cut-point
//     Padua 2015, J Athl Train 50:589) — supportive criterion (graded only when entered).
//
...
//     ikdcOrKosPercent, aclRsi, pathwayType }
// pathwayType ('surgical' | 'conservative', default 'surgical') only relabels
// the time criterion (surgery vs injury); the 9-month threshold itself is from
// post-ACLR cohorts and is an extrapolation for non-operative pathways.
...
export function evaluateRTSBattery(input = {}) {
  ...
  const time = gradedCriterion({
    key: 'time',
    label: pathwayType === 'conservative' ? 'Time since injury' : 'Time since surgery',
    value: monthsSinceSurgery,
  ...
  const landing = gradedCriterion({
    ...
    threshold: `< ${T.lessErrors} errors`, unit: '/19', source: 'Padua 2009; Padua 2015',
  })
  ...
  if (!hardCore.every(c => c.assessed)) {
    ...
    summary = 'Objective core criteria met (9-month gate, quad LSI, hop battery, effusion).'
  } else {
