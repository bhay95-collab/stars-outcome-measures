...
import { followUpAttentionLabel, formatFollowUpDate, responseOneLine, summarizeFollowUpRecords } from '../followups'
...
export async function exportPatientSummaryPdf({ patient, assessments, followups = [] }) {
  ...
  if (followUpSummary.latestResponse?.response) {
    const response = followUpSummary.latestResponse.response
    paragraph(`${followUpAttentionLabel(response.attention_level)} - ${responseOneLine(response)}`, { color: toneColor(response.attention_level) })
    paragraph(`Last response: ${formatFollowUpDate(response.created_at)}. Pending links: ${followUpSummary.pendingCount}. Overdue links: ${followUpSummary.overdueCount}.`)
    if (response.concern_text) paragraph(`Patient concern: ${response.concern_text}`)
  } else {
