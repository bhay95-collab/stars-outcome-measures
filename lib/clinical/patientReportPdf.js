...
const AMBER = rgb(0.541, 0.396, 0.071)
...
export async function exportPatientSummaryPdf({ patient, assessments, followups = [] }) {
  ...
  if (!summary.entries.length) {
    ...
    summary.entries.forEach(entry => {
      ...
      page.drawText(
        entry.latest.results?.meta?.patientReported
          ? `${fmtDate(entry.latest.created_at)} (patient-reported)`
          : fmtDate(entry.latest.created_at),
        { x: margin + 330, y, size: 9, font: regular, color: MUTED },
      )
  ...
  if (followUpSummary.latestResponse?.response || followUpSummary.latestResponse?.assessment) {
    ...
    if (record.assessment?.comparison?.summaryLine) {
      paragraph(record.assessment.comparison.summaryLine, {
        color: record.assessment.comparison.meaningfulDecline ? RED : MUTED,
      })
    }
