...
export const SUMMARY_DOMAINS = [
  ...
  },
  {
    ...
    measureIds: ['FSS', 'HADS', 'PDQ8', 'RPQ', 'BIVI', 'FABQ'],
  },
  {
    id: 'pain',
    label: 'Pain',
    measureIds: ['NPRS', 'BPFS'],
  },
  {
    id: 'lowerLimbMsk',
    label: 'Lower Limb Function',
    measureIds: ['LEFS', 'KOOS', 'HOOS', 'HAGOS', 'FAAM', 'CAIT', 'ATRS', 'OMAS', 'QuadLSI', 'HopBattery', 'LESS', '30STS', 'FTSTS'],
  },
  {
    id: 'upperLimbMsk',
...
function buildLatestEntries(groups, condition) {
  return Object.keys(MEASURES)
    ...
    .map(measureId => {
      ...
      const mcid = measure.mcidKey && previous && numericValue != null && toFiniteNumber(previous.results?.primaryValue) != null
        ? getMCIDStatus(measure.mcidKey, numericValue, toFiniteNumber(previous.results.primaryValue), condition)
        : null
...
const PLAIN_DOMAIN_LABELS = {
  ...
  pain: 'pain',
  lowerLimbMsk: 'lower limb function',
  upperLimbMsk: 'upper limb function',
  patientGoals: 'patient-chosen activities',
}
...
const PLAIN_MEASURE_AREAS = {
  ...
  NPRS: 'pain intensity',
  PSFS: 'patient-chosen activities',
  LEFS: 'lower limb function',
  BPFS: 'back-related function',
  KOOS: 'knee function',
  HOOS: 'hip function',
  FAAM: 'foot and ankle function',
  '30STS': 'sit-to-stand strength',
  FTSTS: 'sit-to-stand speed',
...
export function buildPatientSummary(patient, assessments = []) {
  ...
  const entries = buildLatestEntries(groups, condition)
...
export function buildTodayAssessmentRows(patient, assessments = [], date = new Date()) {
  ...
  return summary.entries
    ...
    .map(entry => ({
      measureId: entry.measureId,
      measure: entry.latest.results?.meta?.patientReported
        ? `${entry.measure.name} (patient-reported)`
        : entry.measure.name,
...
export function buildMeasureTrendSeries(groups, measureId) {
  ...
  // Primary series + any registry-declared subscales (results.meta[key]).
  // Subscale definitions live in measures.js so every consumer stays in sync.
  const series = [
    {
      key: 'primary',
    ...
    ...(measure.subscales ?? []).map(subscale => ({
      key: subscale.key,
      label: subscale.label,
      unit: subscale.unit ?? measure.primaryUnit ?? '',
      mcidKey: subscale.mcidKey ?? null,
      values: records.map(record => ({ date: record.created_at, value: toFiniteNumber(record.results?.meta?.[subscale.key]), record })),
    })),
  ]
