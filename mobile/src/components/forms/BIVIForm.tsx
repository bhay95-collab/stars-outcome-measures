...
import { calcBIVI, BIVI_ITEMS } from '@clinical/bivi';
...
// Response labels from bivi.js clinical spec: 0=Not at all, 1=A little, 2=Quite a lot, 3=A great deal
const BIVI_OPTIONS: ChipOption[] = [
...
const BIVI_SCALE_KEY: ScaleKeyEntry[] = BIVI_OPTIONS.map(o => ({
  value: o.value,
...
export function BIVIForm({ patientId }: { patientId: string }) {
  const insets = useSafeAreaInsets();
  const [patient, setPatient] = useState<Patient | null>(null);
  const [scores, setScores] = useState<(number | null)[]>(Array(ITEM_COUNT).fill(null));
  const [result, setResult] = useState<BIVIResult | null>(null);
  const { user } = useAuth();
  ...
  function handleScore(idx: number, value: number) {
    ...
    if (next.every(s => s !== null)) {
      setResult(calcBIVI({ items: next }) as BIVIResult | null);
    } else {
      setResult(null);
    }
  }
  ...
  return (
    <Screen padded={false} rootBackground={colors.primaryDark} safeEdges={['top', 'left', 'right']}>
      <NavyHeader mode="nav" leftLabel="‹" onLeft={() => router.back()} title="BIVI" />
      ...
      >
        ...
        <ScaleKey entries={BIVI_SCALE_KEY} />
        ...
        {result !== null ? (
          <View style={styles.resultCard}>
            <View style={styles.resultHeaderRow}>
              <View>
                <Text style={styles.resultMeta}>BIVI RESULT</Text>
