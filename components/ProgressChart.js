    ...
    <>
      ...
      <div className="chart-wrap">
        <svg viewBox={`0 0 ${W} ${H}`} className="chart-svg" aria-hidden="true">
          ...
          <text x={PAD.left - 6} y={toY(yMax)} fontSize="10" fill="#8A96A3" textAnchor="end" dominantBaseline="middle">{yMax}</text>
          <text x={PAD.left - 6} y={toY(yMin)} fontSize="10" fill="#8A96A3" textAnchor="end" dominantBaseline="middle">{yMin}</text>
          <text
            fontSize="9"
            fill="#8A96A3"
            textAnchor="middle"
          ...
          {data.map((d, i) => (
            <g key={i}>
              <circle cx={toX(i)} cy={toY(d.value)} r="4" fill="#236499" stroke="#ffffff" strokeWidth="2" />
              <text x={toX(i)} y={toY(d.value) - 10} fontSize="10" fill="#1F2933" textAnchor="middle" fontWeight="600">
                {d.value.toFixed(2)}
              </text>
              <text x={toX(i)} y={PAD.top + INNER_H + 16} fontSize="10" fill="#8A96A3" textAnchor="middle">
                {fmtDateShort(d.date)}
              </text>
            </g>
          ))}
