...
16. **`acl_pathways` is the first stateful per-patient table** — every other clinical record is a stateless `assessments` row. The ACL Phase Tracker stores one pathway row per patient (`current_phase`, `phase_history`); gate tests are read live from `assessments`, never duplicated. Apply migration `20260628000000_add_acl_pathways.sql` before deploying web. The clinician advances the phase — the tool never auto-progresses and never issues an RTS clearance (clinical-safety rule). ACL-RSI/IKDC are stubbed `pending` in the RTS battery until licensed; no substitute cut-off is invented.
